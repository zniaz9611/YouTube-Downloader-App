...
class MyApp(ctk.CTk):
    ...
    def fetch_video_info(self, event=None):
        try:
            ...
            # Populate a quality menu with only progressive streams
            resolutions = [stream.resolution for stream in
                           self.yt_object.streams.filter(progressive=True, file_extension='mp4')]

            if resolutions:
                self.quality_menu.configure(values=resolutions, state="normal")
                self.quality_var.set(resolutions[-1])  # Set default to highest
            else:
    ...
    def download_video(self):
        ...
        try:
            ...
            if self.audio_only_var.get() == "on":
                ...
                stream = self.yt_object.streams.filter(progressive=True, resolution=quality).first()
            ...
            if stream:
                ...
                self.status_label.configure(text="Selected quality not available.", text_color="red")
